...
const JPEG_MINIMAL = [
  ...
  0x01, 0x00, 0x00, 0xff, 0xc0, 0x00, 0x0b, 0x08, 0x00, 0x01, 0x00, 0x01, 0x01, 0x01, 0x11, 0x00, 0xff,
  0xd9
]
...
describe("ArticleService.create", () => {
  ...
  test("surfaces a per-locale slug collision as a field-level conflict", async () => {
    ...
    expect(payload.errors).toEqual([
      expect.objectContaining({ path: ["slug"], message: "Slug already exists" })
    ])
    expect(await countArticles()).toBe(before)
  ...
  test("rejects an unknown category id without leaking a server error", async () => {
    ...
    const payload = validationPayload(
      await service.create(createBody({ categoryId: faker.string.uuid({ version: 7 }) })).catch((error: unknown) => error)
    )
